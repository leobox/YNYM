# === 모바일 주식 패턴 스캐너 ===
# 단일 셀에 붙여넣고 실행하세요. 로컬 import 없이 동작합니다.
# 점수는 상승 확률이 아닙니다. 매수/매도는 직접 MTS에서 판단하세요.
# 돌파선은 추천 매수가가 아닙니다.
...
RELEASE_ID = 'pattern-colab-2026-09-17'
...
def entry_context(current, close):
...
def mobile_table_html(table):
    ...
    for r in table.to_dict('records'):
        ...
        state='다음 봉 마감 대기' if watch else '패턴 조건 충족'
...
def save_snapshot_run(directory, stamp, now, stocks, rows, result, watch, errors):
    """Atomically save four tables and a manifest for this invocation."""
    ...
    for label,table in tables.items():
        target=directory/f'{stamp}_{label}.csv'
        with tempfile.NamedTemporaryFile('w',delete=False,dir=directory,suffix='.tmp',
                                         encoding='utf-8-sig',newline='') as handle:
            table.to_csv(handle,index=False)
            temporary=handle.name
        os.replace(temporary,target)
        files[target.name]={'rows':len(table),'sha256':hashlib.sha256(target.read_bytes()).hexdigest()}
    manifest={
        'run_id':stamp,'fetched_at':now.isoformat(),'code':{'release_id':RELEASE_ID,'dirty':None},
        'config':{'scan_limit':SCAN_LIMIT,'top_n':TOP_N,'workers':WORKERS,'mcap_min':MCAP_MIN,
                  'mcap_max':MCAP_MAX,'min_price':MIN_PRICE,'volume_weight':VOLUME_WEIGHT,
                  'max_extension_atr':MAX_EXTENSION_ATR},
        'data':{'pattern_source':'Yahoo Finance 60m chart endpoint',
                'quote_source':'Naver mobile stock endpoints','timezone':'Asia/Seoul',
                'adjustment_basis':'not verified','historical_universe':False},
        'metadata':{'total_scanned':len(stocks),'scored':len(rows),'errors':len(errors)},
        'files':files,
    }
    target=directory/f'manifest_{stamp}.json'
    with tempfile.NamedTemporaryFile('w',delete=False,dir=directory,suffix='.tmp',encoding='utf-8') as handle:
...
def run():
    ...
    if SAVE_SNAPSHOTS:
        ...
        try:
            ...
            manifest=save_snapshot_run(directory,stamp,now,stocks,rows,result,watch,errors)
            saved_note=f'조회 기록: {directory} · {manifest.name} (런타임 초기화 전 내려받아 보관하세요)'
        except OSError as e:
    ...
    print(f'패턴 충족 {len(result)}개 · 관찰 {len(watch)}건')
